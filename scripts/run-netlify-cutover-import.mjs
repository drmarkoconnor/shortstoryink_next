...
assert.ok(['https://migration-cutover-v2--storyink.netlify.app','https://shortstory.ink'].includes(target))
...
await writeFile(`${directory}/${target.includes('shortstory.ink') ? 'production' : 'rehearsal'}-cutover-report.private.json`, JSON.stringify({ target, checkedAt: new Date().toISOString(), status: response.status, ...result }, null, 2), { mode: 0o600 })
