...
async function createSubmissionAction(formData: FormData) {
	...
	if (mode === 'modern') {
		if (!workshopId) {
			redirect('/app/writer?error=Please+select+a+workshop.')
		}
		...
		if (membershipError) {
			redirect('/app/writer?error=Unable+to+validate+workshop+membership.')
		}

		if (!membership) {
			redirect(
				'/app/writer?error=You+can+only+submit+to+your+assigned+workshops.',
			)
		}
...
export default async function WriterPage({
	...
	if (mode === 'modern') {
		...
		if (membershipError) {
			...
			if (error) {
				workshopError = 'Unable to load workshop details.'
			} else {
		...
		if (error) {
			...
			submissions = (
				...
				workshopTitle:
					workshopTitleById[submission.workshop_id] ?? 'Workshop unknown',
			}))
		}
	} else {
		const { data: submissionRows, error } = await supabase
		...
		if (error) {
			...
			submissions = (
				...
				workshopTitle: 'General writer queue',
			}))
		}
	}
		...
		<section className="space-y-5">
			<div className="surface p-6 lg:p-8">
				...
				<p className="muted mt-3 max-w-prose text-sm leading-relaxed">
					Start a new piece, keep track of what is waiting in the review loop,
					and return to feedback once it has been published.
				</p>
			</div>
			...
			<div className="grid gap-4 sm:grid-cols-3">
				<div className="surface p-5">
					...
					<p className="mt-2 text-3xl text-parchment-100">{submittedCount}</p>
				</div>
				<div className="surface p-5">
					<p className="text-xs uppercase tracking-[0.12em] text-silver-300">
						In review
					</p>
					<p className="mt-2 text-3xl text-parchment-100">{inReviewCount}</p>
				</div>
				<div className="surface p-5">
					<p className="text-xs uppercase tracking-[0.12em] text-silver-300">
						Published feedback
					</p>
					<p className="mt-2 text-3xl text-parchment-100">{publishedCount}</p>
				</div>
			</div>
			...
			<div className="surface p-6 lg:p-8">
				...
				<p className="muted mt-3 max-w-prose text-sm leading-relaxed">
					Paste the piece as you want it read. Formatting is preserved, and the
					body editor now keeps Tab indentation inside the manuscript.
				</p>
				...
				{isWorkshopRequired && !workshopError && workshops.length === 0 && (
					<p className="mt-4 rounded-lg border border-amber-300/30 bg-amber-300/10 px-3 py-2 text-sm text-amber-100">
						No workshop membership found yet. Teacher/admin must assign you to a
						workshop.
					</p>
				)}
				{!isWorkshopRequired && (
					<p className="mt-4 rounded-lg border border-sky-300/30 bg-sky-300/10 px-3 py-2 text-sm text-sky-100">
						...
						general writer queue until workshop tables are migrated.
					</p>
				)}
					...
					<div className="grid gap-3 md:grid-cols-2">
						...
						{isWorkshopRequired ? (
							...
							<div className="rounded-xl border border-white/10 bg-ink-900/35 px-4 py-2.5 text-sm text-silver-200">
								General writer queue (no workshop selection in legacy mode)
							</div>
						)}
					</div>
						...
						{submissions.map((submission) => (
							<li
								...
								<p className="mt-2 text-xs text-silver-300">
									{submission.workshopTitle ?? 'General writer queue'} {' · '}
